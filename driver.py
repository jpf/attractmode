...
def environ_cb(cmd, data):
	...
	if cmd == 2: # RETRO_ENVIRONMENT_GET_OVERSCAN
		pass

	elif cmd == 8: # RETRO_ENVIRONMENT_SET_PERFORMANCE_LEVEL
		level = cast(data, POINTER(c_uint)).contents.value
		assert level == 7

	elif cmd == 10: # RETRO_ENVIRONMENT_SET_PIXEL_FORMAT
		print "PIXEL FORMAT"
		format = cast(data, POINTER(c_int)).contents.value
		print "pixel format:", format
		assert format == 2 # 2 is RETRO_PIXEL_FORMAT_RGB565
		return True

	elif cmd == 11: # RETRO_ENVIRONMENT_SET_INPUT_DESCRIPTORS
	...
	elif cmd == 16: # RETRO_ENVIRONMENT_SET_VARIABLES
		print "VARIABLES"
	...
	elif cmd == 17: # RETRO_ENVIRONMENT_GET_VARIABLE_UPDATE
		pass

	elif cmd == 27: # RETRO_ENVIRONMENT_GET_LOG_INTERFACE
		pass

	elif cmd == 35: # RETRO_ENVIRONMENT_SET_CONTROLLER_INFO
		pass

	elif cmd == 65578: # RETRO_ENVIRONMENT_SET_SUPPORT_ACHIEVEMENTS
		pass
...
while not simulation_done:
	...
	if step >= 3000 and step % 5 == 0:
		...
		arr = np.frombuffer(last_framebuffer, dtype=np.uint16, count=512*1024).reshape((512,1024))[0:224,0:256].astype(np.uint32)
		...
		#	hold=False)
		plt.imshow(
			np.frombuffer(serialize_buffer,dtype='>u2')[20000:26400].reshape(-1,32),
			interpolation='nearest',
			hold=False)
		plt.pause(0.01)
